...
def check_answer():
    ...
    print(f'Статистика\n- Всего вопросов: {window.cur_question}\n- Правильный ответов: {right_answer_user}\n- Рейтинг: {right_answer_user / (window.cur_question ) * 100}%')
...
def next_question():
    ...
    if window.cur_question-1 >= len(questions_list):
        window.cur_question = 1
        right_answer_user = 0
        shuffle(questions_list)
    q = questions_list[window.cur_question-1]
    ask(q)
